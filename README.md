...
- A Mac (Apple Silicon assumed) with Homebrew. macOS 26+ for the default container runtime (**Apple `container`**, installed for you; Khoj runs with no internet egress). On older macOS it falls back to **Colima**; set `CONTAINER_RUNTIME=docker` to use Docker Desktop instead.
